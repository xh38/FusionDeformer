...
class SparseMat:
    ...
    def to_coo(self):
        return coo_matrix((self.vals.cpu(), (self.inds[0, :].cpu(), self.inds[1, :].cpu())), shape=(self.n, self.m))
...
class SPLUSolveLayer(torch.autograd.Function):
    ...
    def forward(ctx, solver, b):
        ...
        assert 1 <= b.shape[-1] <= 3, f'got shape {b.shape} expected last dim to be in range 1-3'
        b = b.contiguous()
        ctx.solver = solver
